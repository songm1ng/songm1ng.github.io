...
if (document.all)document.write('<a href="#" onClick="bookmarkit();" title="把“Pcook CMS 泡客”加入您的收藏夹！">收藏本站</a>')
        ...
        <a target="_blank" href="#" class="end">网站地图</a>
        </span>
		</div>
...
<!-- 一个循环段落开始 -->
		...
		<div>
		<span class="mbg3">
		<span class="title">您现在的位置：<a href="Index.asp">首页</a> >> <a href="Class.asp?ID=<%=rs("ClassID")%>"><%=ClassName%></a> >> 内容</span>
	...
	<%If ""&IsPing&""=1 then%>
		...
		<div class="lpic2"></div>
			<table border="0" cellspacing="0" cellpadding="2" width="600" align="center"><tr>
					...
					<td>大名：<input name="memAuthor" type="text" id="memAuthor" maxlength="8" class="borderall1" value="<%If pcookuserName<>"" then Response.Write(""&pcookuserName&"") else Response.Write("匿名泡友") End if%>"/>
					</td><tr><td>
					<li>内容：<textarea name="memContent" cols="30" rows="8" style="width:400px;height:150px;" wrap="virtual" id="memContent" class="borderall1"/></textarea>
					</td><tr><td>
...
a1=headrs("Title")
if seo=1 then 
response.Write("<li class='titles'>上一篇：<a href='"&SitePath&"Article_"&a0&".html'>"&a1&"</a></li>") 
elseif seo=2 then 
response.Write("<li class='titles'>上一篇：<a href='"&SitePath&"Article/?"&a0&".html'>"&a1&"</a></li>")  
elseif seo=3 then 
response.Write("<li class='titles'>上一篇：<a href='"&SitePath&"List.asp?ID="&a0&"'>"&a1&"</a></li>") 
elseif seo=4 then 
response.Write("<li class='titles'>上一篇：<a href='"&SitePath&"Article/"&a0&".html'>"&a1&"</a></li>") 
end if
end if 
...
a1=newrs("Title")
if seo=1 then 
response.Write("<li class='titles'>下一篇：<a href='"&SitePath&"Article_"&a0&".html'>"&a1&"</a></li>") 
elseif seo=2 then 
response.Write("<li class='titles'>下一篇：<a href='"&SitePath&"Article/?"&a0&".html'>"&a1&"</a></li>") 
elseif seo=3 then 
response.Write("<li class='titles'>下一篇：<a href='"&SitePath&"List.asp?ID="&a0&"'>"&a1&"</a></li>") 
elseif seo=4 then 
response.Write("<li class='titles'>下一篇：<a href='"&SitePath&"Article/"&a0&".html'>"&a1&"</a></li>") 
end if 
end if 
